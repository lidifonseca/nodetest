...
export class OperadoraUpdate extends React.Component<IOperadoraUpdateProps, IOperadoraUpdateState> {
  ...
  componentDidUpdate(nextProps, nextState) {
    ...
    if (
      ...
      nextProps.operadoraEntity.unidadeEasy &&
      nextProps.operadoraEntity.unidadeEasy.id
    ) {
      this.setState({
        unidadeEasySelectValue: nextProps.unidadeEasies.map(p =>
          nextProps.operadoraEntity.unidadeEasy.id === p.id ? { value: p.id, label: p.razaoSocial } : null
        )
      });
    }

    if (
      nextProps.tipoOperadoras.length > 0 &&
      this.state.tipoOperadoraSelectValue === null &&
      nextProps.operadoraEntity.tipoOperadora &&
      nextProps.operadoraEntity.tipoOperadora.id
    ) {
      this.setState({
        tipoOperadoraSelectValue: nextProps.tipoOperadoras.map(p =>
          nextProps.operadoraEntity.tipoOperadora.id === p.id ? { value: p.id, label: p.tipo } : null
        )
      });
    }
  }
  ...
  saveEntity = (event: any, errors: any, values: any) => {
    if (errors.length === 0) {
      ...
      const entity = {
        ...
        tipoOperadora: this.state.tipoOperadoraSelectValue ? this.state.tipoOperadoraSelectValue['value'] : null,
        ...values
      };
