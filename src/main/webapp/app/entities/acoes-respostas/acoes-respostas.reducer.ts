...
export const getEntities: ICrudGetAllActionAcoesRespostas<IAcoesRespostas> = (
  ...
  const respostasIdRequest = respostasId ? `respostasId.equals=${respostasId}&` : '';
  ...
  return {
    ...
    payload: axios.get<IAcoesRespostas>(
      `${requestUrl}${abrirCampoPersonalizadoRequest}${condicaoSexoRequest}${observacoesRequest}${tipoCampo1Request}${tipoCampo2Request}${respostasIdRequest}${perguntasQuestionarioIdRequest}cacheBuster=${new Date().getTime()}`
    )
  };
};
...
export const createEntity: ICrudPutAction<IAcoesRespostas> = entity => async dispatch => {
  entity = {
    ...entity,
    respostasId: entity.respostasId === 'null' ? null : entity.respostasId,
